...
// Dev-only: store the last magic link URL per email so Electron can retrieve it
export const devMagicLinks = new Map<string, { url: string; ts: number }>();
    ...
    magicLink({
      sendMagicLink: async ({ email, url }) => {
        if (process.env.NODE_ENV !== "production") {
          ...
          devMagicLinks.set(email, { url, ts: Date.now() });
          return;
        }
